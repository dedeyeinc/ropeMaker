...
ropeMaker_ribbonizer.py 
version 0.0.1
copyright (C) 2024 Mathias Villalobos
...
def get_selection():
    ...
    if shapes and mc.objectType(shapes[0]) == "nurbsSurface":
        surf_tr = sel[0]
    elif shapes and mc.objectType(shapes[0]) == "nurbsCurve":
        surf_tr = sel[0]
    ...
    # so instead of raising an error we just get the connected surface
        ...
        mc.warning("dude, that isn't a nurbs surface or nurbs curve")
        return
...
def ribbonize(surf_tr, equal=1, num_of_ctrls=5, num_of_jnts=29, prefix="", constrain=1, add_fk=0, wire=0):
    ...
    #####################################################
    shapes = mc.listRelatives(surf_tr, s=True)
    if shapes and mc.objectType(shapes[0]) == "nurbsCurve":
        print(shapes[0])
        name = mc.textField(ropeMakerUI.tf1,q=1,tx=1)
        extrude_curve = mc.curve(d=1,p=([-1,0,0],[1,0,0]),k=(0,1),name='extrude_curve')
        mc.select('extrude_curve',r=1)
        mc.select(surf_tr,add=1)
        
        surf_tr = mc.rename(surf_tr, prefix + "ribbon_curve")
        ...
        mm.eval('Extrude')
        #mc.extrude("extrude_curve",surf_tr,ch=1,rn=0,po=0,et=2,ucp=0,fpt=0,upn=1,rotation=0,scale=1,rsp=1)
        mc.setAttr("extrude1.fixedPath",1)
        mc.setAttr("extrude1.useComponentPivot",1)
        
        #mc.rename("extrudedSurface1", prefix + "ribbon_curve")
        surf_tr = mc.rename("extrudedSurface1", prefix + "ribbon_surface")
        surf = mc.listRelatives(surf_tr, shapes=True)[0]
        
        # freeze transformations and delete the surface history
        #mc.makeIdentity(surf_tr, t=True, r=True, s=True, apply=True)
        mc.delete(surf_tr, ch=True)
        mc.delete("extrude_curve")
...
class UI(object):
    ...
    def __init__(self):
        ...
        mainWindow = mc.window(windowName, wh=(windowWidth, windowHeight), t="Rope Ribbonizer", s=False, mnb=False, mxb=False)
